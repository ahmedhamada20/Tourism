                    ...
                    <div class="card-body text-center">
                        ...
                        <div class="table-responsive">
                            <table class="table table-bordered mb-0">
                                ...
                                @auth
                                    @if (auth()->user()->countries_id == 173)
                                        <tr>
                                            <td>1</td>
                                            <td>{{ $newCart_admin->transfer->name }}</td>
                                            <td>{{ $newCart_admin->transfer->price_EG_go }}</td>
                                            <td>{{\Gloudemans\Shoppingcart\Facades\Cart::subtotal()}}</td>
                                            <td>{{\Gloudemans\Shoppingcart\Facades\Cart::subtotal() + $newCart_admin->transfer->price_EG_go}}</td>
                                    ...
                                    @else
                                        <tr>
                                            <td>1</td>
                                            <td>{{ $newCart_admin->transfer->name }}</td>
                                            <td>{{ $newCart_admin->transfer->price_EN_go }}</td>
                                            <td>{{\Gloudemans\Shoppingcart\Facades\Cart::subtotal()}}</td>
                                            <td>{{\Gloudemans\Shoppingcart\Facades\Cart::subtotal() + $newCart_admin->transfer->price_EN_go}}</td>
                            ...
                            <button type="button" class="btn btn-primary mt-2" data-bs-toggle="modal"
                                    data-bs-target="#exampleModal{{$newCart_admin->id}}">
                                Check Out
                            </button>

                            <!-- Modal -->
                            <div class="modal fade" id="exampleModal{{$newCart_admin->id}}" tabindex="-1"
                                    ...
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-body">
                                            <form action="{{route('order_store')}}" method="post">
                                                ...
                                                <input type="hidden" name="cart_id" value="{{$newCart_admin->id}}">
                                                <p class="text-danger">{{__('app.Check_Out')}}</p>
                                                @auth
                                                    @if (auth()->user()->countries_id == 173)
                                                        <input type="text" readonly class="form-control"
                                                               value="{{ (int) \Gloudemans\Shoppingcart\Facades\Cart::subtotal() +  $newCart_admin->transfer->price_EG_go }}">
                                                    @else
                                                        <input type="text" readonly class="form-control"
                                                               value="{{ (int) \Gloudemans\Shoppingcart\Facades\Cart::subtotal() +  $newCart_admin->transfer->price_EN_go}}">
                                                    @endif
                                                @else
                                                    <input type="text" readonly class="form-control"
                                                           value="{{ (int) \Gloudemans\Shoppingcart\Facades\Cart::subtotal() +  $newCart_admin->transfer->price_EN_go}}">
