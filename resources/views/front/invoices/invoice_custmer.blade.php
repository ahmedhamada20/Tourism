                                ...
                                @auth
                                    @if (auth()->user()->countries_id == 173)
                                        <tr>
                                            ...
                                            <td>{{ $newCart_cutsmer->transfer->price_EG_go  }}</td>
                                            <td>{{\Gloudemans\Shoppingcart\Facades\Cart::subtotal()}}</td>
                                            <td>{{\Gloudemans\Shoppingcart\Facades\Cart::subtotal() + $newCart_cutsmer->transfer->price_EG_go  }}</td>
                                    ...
                                    @else
                                        <tr>
                                            <td>1</td>
                                            <td>{{ $newCart_cutsmer->transfer->name   }}</td>
                                            <td>{{ $newCart_cutsmer->transfer->price_EN_go   }}</td>
                                            <td>{{\Gloudemans\Shoppingcart\Facades\Cart::subtotal()}}</td>
                                            <td>{{\Gloudemans\Shoppingcart\Facades\Cart::subtotal() + $newCart_cutsmer->transfer->price_EN_go }}</td>
                                                ...
                                                @auth
                                                    @if (auth()->user()->countries_id == 173)
                                                        <input type="text" readonly class="form-control"
                                                               value="{{ (int) \Gloudemans\Shoppingcart\Facades\Cart::subtotal() +  $newCart_cutsmer->transfer->price_EG_go }}">
                                                    @else
                                                        <input type="text" readonly class="form-control"
                                                               value="{{ (int) \Gloudemans\Shoppingcart\Facades\Cart::subtotal() +  $newCart_cutsmer->transfer->price_EN_go}}">
                                                    @endif
                                                @else
                                                    <input type="text" readonly class="form-control"
                                                           value="{{ (int) \Gloudemans\Shoppingcart\Facades\Cart::subtotal() +  $newCart_cutsmer->transfer->price_EN_go}}">
